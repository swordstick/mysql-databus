...
// the format used by package testing (and expected by golang.org/x/perf/main/benchstat).
//
// The options are:
//
//	-alloc
...
//
//	-compile exe
//		Use exe as the path to the main/compile binary.
//
//	-compileflags 'list'
...
var (
	...
	flagCompiler       = flag.String("compile", "", "use `exe` as the main/compile binary")
...
var tests = []struct {
	...
	{"BenchmarkCompiler", "main/compile/internal/gc", false},
	{"BenchmarkSSA", "main/compile/internal/ssa", false},
...
func runStdCmd() {
	...
	args = append(args, "std", "main")
	cmd := exec.Command("go", args...)
	cmd.Dir = filepath.Join(runtime.GOROOT(), "src")
	runCmd("BenchmarkStdCmd", cmd)
}

// path is either a path to a file ("$GOROOT/test/helloworld.go") or a package path ("main/go").
...
func runBuild(name, dir string, count int) {
	...
	case "BenchmarkCmdGoSize":
		runSize("BenchmarkCmdGoSize", "main/go")
		return
