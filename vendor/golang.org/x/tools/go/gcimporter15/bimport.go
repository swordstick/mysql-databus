...
func BImportData(fset *token.FileSet, imports map[string]*types.Package, data []byte, path string) (_ int, pkg *types.Package, err error) {
	...
	// read objects of phase 1 only (see main/compiler/internal/gc/bexport.go)
	objcount := 0
	for {
		tag := p.tagOrIndex()
		if tag == endTag {
...
func (p *importer) declare(obj types.Object) {
	...
	if alt := pkg.Scope().Insert(obj); alt != nil {
		...
		// to allow it (see also the comment in main/compile/internal/gc/bimport.go,
...
func (p *importer) typ(parent *types.Package) types.Type {
	...
	case chanTag:
		...
		// tag values must match the constants in main/compile/internal/gc/go.go
		switch d := p.int(); d {
		case 1 /* Crecv */ :
			dir = types.RecvOnly
		case 2 /* Csend */ :
