...
package vet
`,
		"src/main/go/doc.go": `// The go command
package main
`,
		"src/main/gofmt/doc.go": `// The gofmt command
package main
`,
		"src/main/vet/vet.go": `// The vet command
package main
`,
	})
	fs := make(vfs.NameSpace)
...
{{comment_text .Doc "    " "\t"}}{{end}}{{end}}{{end}}`))
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			args: []string{"vet"},
			exp:  "use 'godoc main/vet' for documentation on the vet command \n\nPACKAGE Package vet\n",
		},
		{
			desc: "root directory",
			args: []string{"/"},
